...
_ROBOT_BODY_GLYPH = '\u25a0'  # ■
_ROBOT_BODY_STYLE = 'bold cyan'

ROBOT_HALF_C = 4  # half-width  → 9 cells wide total
ROBOT_HALF_R = 2  # half-height → 5 cells tall total

def robot_body_glyph(dr: int, dc: int, theta_deg: float) -> tuple[str, str] | None:
    if abs(dr) > ROBOT_HALF_R or abs(dc) > ROBOT_HALF_C:
        return None
    if dr == 0 and dc == 0:
        return robot_glyph(theta_deg), _STYLE_ROBOT
    if abs(dr) == ROBOT_HALF_R or abs(dc) == ROBOT_HALF_C:
        return _ROBOT_BODY_GLYPH, _ROBOT_BODY_STYLE
    return None
#edited code
