...
#this is to check for collision(edited)
def _check_collision(
    mapbytes: bytearray,
    x_mm: float, y_mm: float, theta_deg: float,
) -> bool:
    """Return True if any obstacle is within COLLISION_STOP_MM in front."""
    import math
    from settings import (
        COLLISION_STOP_MM, COLLISION_ARC_DEG,
        MAP_SIZE_PIXELS, MAP_SIZE_METERS, UNKNOWN_BYTE,
    )
    px_per_mm = MAP_SIZE_PIXELS / (MAP_SIZE_METERS * 1000.0)
    steps = 10  # number of range steps to probe

    for arc in range(-COLLISION_ARC_DEG // 2, COLLISION_ARC_DEG // 2 + 1, 5):
        angle_rad = math.radians(theta_deg + arc)
        for step in range(1, steps + 1):
            dist = COLLISION_STOP_MM * step / steps
            probe_x = x_mm + dist * math.cos(angle_rad)
            probe_y = y_mm + dist * math.sin(angle_rad)
            col = int(probe_x * px_per_mm)
            row = int(MAP_SIZE_PIXELS - 1 - probe_y * px_per_mm)
            if not (0 <= col < MAP_SIZE_PIXELS and 0 <= row < MAP_SIZE_PIXELS):
                continue
            val = mapbytes[row * MAP_SIZE_PIXELS + col]
            if val < 100:   # confirmed wall/obstacle (BreezySLAM: 0=wall)
                return True
    return False
#unil here
        ...
        for raw_angles, raw_distances in lidar_driver.scan_rounds(lidar, scan_mode):
            ...
            if now - last_map_update >= MAP_UPDATE_INTERVAL:
                ...
                last_map_update = now
                        ...
                        # ← ADD HERE
            collision = _check_collision(mapbytes, x_mm, y_mm, theta_deg)
            pss.collision_imminent.value = collision #edited

#edited=====================
            if collision:
                with open('/tmp/slam_collision', 'w') as f:
                    f.write('1')
            else:
                try:
                    import os
                    os.remove('/tmp/slam_collision')
