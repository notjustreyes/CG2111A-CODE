...
from renderer import (
    ...
    robot_body_glyph, ROBOT_HALF_R, ROBOT_HALF_C,   # ← add this line (edited code)
)
...
#edited code============================================
                if (robot_visible
                        and abs(sr - robot_sr) <= ROBOT_HALF_R
                        and abs(sc - robot_sc) <= ROBOT_HALF_C):
                    body = robot_body_glyph(
                        sr - robot_sr, sc - robot_sc, snapshot['theta_deg']
                    )
                    if body is not None:
                        if run_glyph:
                            text.append(run_glyph, style=run_style)
                            run_glyph = ''
                        text.append(body[0], style=body[1])
                        continue
#edited code==========================================
