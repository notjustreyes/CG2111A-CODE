...
bool txEnqueue(const uint8_t *data, uint8_t len) {
  // TODO
  //use tx_tail (pos where the next byte is transmitted), tx_head (pos where the next byte is added)
  //before writing into tx_buf, need to see if there is enough space to write the data
  uint8_t free_space = (TX_BUFFER_SIZE) - ((tx_head - tx_tail) & (TX_BUFFER_MASK));  //covers the case where H and T are wrapped to give negative dist from each other
  if (free_space < len)                                                              //free space of tx_buf < len of data to be put in
  {
    return false;
  }
  ...
  for (int i = 0; i < len; i++) {
    ...
    tx_buf[tx_head] = *data;
    tx_head = (tx_head + 1) & TX_BUFFER_MASK;  //increments tx_head after writing to buf
    data++;
  }

  //data is fully transferred to tx_buf, enable UDRIE int
  UCSR0B |= 0b00100000;
  return true;
}
...
ISR(USART0_UDRE_vect) {
  // YOUR CODE HERE
  UDR0 = tx_buf[tx_tail]; //sends 8 byte per ISR call
  uint8_t next = (tx_tail +1) & TX_BUFFER_MASK;  //increments tx_tail with wrap-arround
  tx_tail = next;
...
bool rxDequeue(uint8_t *data, uint8_t len) {
  //want to copy from data to buf
  uint8_t data_len_buf = ((rx_head - rx_tail) & (RX_BUFFER_MASK));  //computes how many spaces USED in the rx_buf

  if (data_len_buf < len)  //if space in rx_buf can fit inside free space of data (len)
  {
    return false;
  }
